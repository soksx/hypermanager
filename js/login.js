...
$(document).ready(function (){
    ...
    $("#add-user-btn").click(function(){
        var fData = $("#add-user").serialize(); 
        var emptyField = false;
        for (var i = 0; i < fData.split('=').length; i++){
            if(fData.split('=')[i].startsWith("&") || fData.split('=')[i].length == 0)
                emptyField = true;
        }
        if (!emptyField){
            $.ajax({
                type: "POST",
                url: "lib/crud/register",
                data: fData,
                datatype: "text",
            ...
            showAlert(0, "Faltan campos por completar");
        }
    });
});
var init = function() {
    ...
    function initHeader() {
        ...
        canvas = document.getElementById('connect-panel');
        canvas.width = width;
...
function showAlert(status = 0, msg){
    ...
    setTimeout(function() {
        if(status == 1)
            $("#close-add-user-btn").click(); //Colapsa el menu de añadir servidor.
        responseDiv.css("height", "30px");
